...
{
    // Check if the chromosome exists in the map
...
std::tuple<std::vector<double>, std::vector<double>> SNPInfo::querySNPs(std::string chr, int64_t start, int64_t end)
{
    ...
    if (this->snp_baf_map.find(chr) == this->snp_baf_map.end()) {
        return std::make_tuple(bafs, pfbs);
    }
