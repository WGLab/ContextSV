...
*/
{
	...
	if (!m) fprintf(stderr, "kc::matrix_new: allocation failure 1");

	/* allocate rows and set pointers to them */
	m[0]=(double *) malloc((size_t)((row*col)*sizeof(double)));
	if (!m[0]) fprintf(stderr, "kc::matrix_new: allocation failure 2");
...
*/
{
	...
	if (!m) fprintf(stderr, "kc::matrixc_new: allocation failure 1");

	/* allocate rows and set pointers to them */
	m[0]=(char *) malloc((size_t)((row*col)*sizeof(char)));
	if (!m[0]) fprintf(stderr, "kc::matrixc_new: allocation failure 2");
...
/* allocate a double vector with subscript range v[0..n-1] */
{
	...
	if (!v) fprintf(stderr, "kc::vector_new: allocation failure");
	return v;
}
...
/* allocate an int vector with subscript range v[nl..nh] */
{
	...
	if (!v) fprintf(stderr, "kc::ivector: allocation failure in ivector()");
	return v-nl+NR_END;
}
...
/* allocate an unsigned char vector with subscript range v[nl..nh] */
{
	...
	if (!v) fprintf(stderr, "kc::cvector: allocation failure in cvector()");
	return v-nl+NR_END;
}
...
/* allocate a int matrix with subscript range m[nrl..nrh][ncl..nch] */
{
	...
	if (!m) fprintf(stderr, "kc::imatrix: allocation failure 1 in matrix()");
	m += NR_END;
	m -= nrl;
	...
	if (!m[nrl]) fprintf(stderr, "kc::imatrix: allocation failure 2 in matrix()");
	m[nrl] += NR_END;
	m[nrl] -= ncl;
...
*/
{
	...
	m=(double **) malloc((size_t) ((nrow+NR_END)*sizeof(double*)));
	if (!m) fprintf(stderr, "kc::submatrix: allocation failure in subdmatrix()");
	m += NR_END;
	m -= newrl;
...
&a[0][0] as the first argument. */
{
	...
	/* allocate pointers to rows */
	m=(double **) malloc((size_t) ((nrow+NR_END)*sizeof(double*)));
	if (!m) fprintf(stderr, "kc::convert_matrix: allocation failure in convert_matrix()");
	m += NR_END;
	m -= nrl;
 ...
 */
{
	...
	if (r2<r1) fprintf(stderr, "kc::matrix_newsubrow: start row (r1) should be less than or equal to end row (r2)");
	if (r1>m) fprintf(stderr, "kc::matrix_newsubrow: start row (r1) should be less than or equal to total row (m)");
	if (r2>m) fprintf(stderr, "kc::matrix_newsubrow: end row (r2) should be less than or equal to total row (m)");
	nrow = r2-r1+1;
	B = matrix_new (nrow, n);
 ...
 */
{
	...
	if (c2<c1) fprintf(stderr, "kc::matrix_newsubcol: start col (c1) should be less than or equal to end col (c2)");
	if (c1>n) fprintf(stderr, "kc::matrix_newsubcol: start col (c1) should be less than or equal to total col (n)");
	if (c2>n) fprintf(stderr, "kc::matrix_newsubcol: end col (c2) should be less than or equal to total col (n)");
	ncol = c2-c1+1;
	B = matrix_new (m, ncol);
 ...
 */
{
	...
	if (r2<r1) fprintf(stderr, "kc::matrix_newsubmat: start row (r1) should be less than or equal to end row (r2)");
	if (r1>m) fprintf(stderr, "kc::matrix_newsubmat: start row (r1) should be less than or equal to total row (m)");
	if (r2>m) fprintf(stderr, "kc::matrix_newsubmat: end row (r2) should be less than or equal to total row (m)");
	if (c2<c1) fprintf(stderr, "kc::matrix_newsubmat: start col (c1) should be less than or equal to end col (c2)");
	if (c1>n) fprintf(stderr, "kc::matrix_newsubmat: start col (c1) should be less than or equal to total col (n)");
	if (c2>n) fprintf(stderr, "kc::matrix_newsubmat: end col (c2) should be less than or equal to total col (n)");
	nrow = r2-r1+1;
	ncol = c2-c1+1;
...
/* delete a row from the matrix and return the new matrix */
{
	...
	if (row>m) fprintf(stderr, "kc::matrix_delRow: the deleted row should be less than the total number of rows of matrix");
	B = matrix_new (m-1, n);
	for (i=0; i<m; i++) {
		for (j=0; j<n; j++) {
			if (i+1>row) {
...
/* delete a column from the matrix and return the new matrix */
{
	...
	if (col>n) fprintf(stderr, "kc::matrix_delCol: the deleted column should be less than the total number of columns of matrix");
	B = matrix_new (m, n-1);
	for (i=0; i<m; i++) {
		for (j=0; j<n; j++) {
			if (j+1>col) {
...
/* normalize a vector to unit Euclidean length */
{
	...
	if (len) {
		...
		fprintf(stderr, "kc::vector_normalize: length of input vector is zero");
	}
}
...
*/
{
	...
	for (i=0;i<n;i++) {
		...
		for (j=0;j<n;j++)
			if (ipiv[j] != 1)
				for (k=0;k<n;k++) {
					...
					} else if (ipiv[k] > 1) fprintf(stderr, "kc::solve: singular matrix-1");
				}
		++(ipiv[icol]);
		if (irow != icol) {
		...
		indxr[i]=irow;
		indxc[i]=icol;
		if (a[icol][icol] == 0.0) fprintf(stderr, "kc::solve: singular matrix-2");
...
*/
{
	...
	*d=1.0;
	for (i=0;i<n;i++) {
		...
		if (big == 0.0) fprintf(stderr, "kc::matrix_ludecomp: singular matrix");
		vv[i]=1.0/big;
	}
	for (j=0;j<n;j++) {
		for (i=0;i<j;i++) {
...
*/
{
    ...
    if (m < n) fprintf(stderr, "kc::matrix_svd: #rows should be greater than or equal to #cols (please create more rows and fill in zero)");
    ...
    {                             /* loop over singular values */
        ...
        {                         /* loop over allowed iterations */
            ...
            if (its >= 100) {
                vector_free (rv1);
                fprintf(stderr, "kc::matrix_svd: No convergence after 100 iterations");
            }
 ...
 */
{
	...
	*nrot=0;
	...
	vector_free (b);
	vector_free (z);
	fprintf(stderr, "kc::matrix_eigen: No convergence after 50 iterations");
}
#undef ROTATE
 ...
 */
{
	...
	if (m<=n) fprintf(stderr, "kc::matrix_reg: the rows must more than columns for regression");
...
void test1 ()
{
	...
	fprintf(stderr, "kc::test1: testing error handling functionality");
}
...
/*inverse standard normal cumulative distribution function, or quantile function, can be expressed in terms of the inverse error function*/
{
	...
	if (x>1 || x<0) fprintf(stderr, "kc::invnormal: x (probability value) should be between 0 and 1");
...
*/
{
	// void fprintf(stderr, char *error_text);
        ...
        if (n <= 1) fprintf(stderr, "kc::moment: n must be at least 2 in moment");
        s=0.0;
        ...
        *sdev=sqrt(*var);
        if (*var) {
        	...
        	fprintf(stderr, "kc::moment: No skew/kurtosis when variance = 0 (in moment)");
}
...
*/
{
	...
	if (n<=0) fprintf(stderr, "Error in kc::bitest: n should be an positive integer");
	if (n<k) fprintf(stderr, "Error in kc::bitest: n should be more than or equal to k");
...
*/
{
	...
	*chi2=0.0;
	for (j=0;j<nbins;j++) {
		if (ebins[j] <= 0.0) fprintf(stderr, "kc::chi2test_onesample: invalid expected number encountered in chi2test_onesample: cell is less than or equal to zero");
...
*/
{
        ...
        if (x < 0.0 || a <= 0.0) fprintf(stderr, "kc::gammp: invalid x or a value");
        if (x < (a+1.0)) {
                gser(&gamser,a,x,&gln);
                return gamser;
        } else {
...
*/
{
        ...
        *gln=gammln(a);
        if (x <= 0.0) {
                if (x < 0.0) fprintf(stderr, "kc::gser: x must be greater than or equal to zero");
                *gamser=0.0;
                return;
        } else {
                ap=a;
                ...
                fprintf(stderr, "kc::gser: a too large or ITMAX too small");
                return;
        }
}
...
*/
{
        ...
        *gln=gammln(a);
        ...
        if (i > ITMAX) fprintf(stderr, "kc::gcf: a too large or ITMAX too small");
...
*/
{
        ...
        if (x < 0.0 || x > 1.0) fprintf(stderr, "kc::betai: Invalid x value (x value must be bewteen 0 and 1)");
...
*/
{
        ...
        if (m > ITMAX) fprintf(stderr, "kc::betacf: a or b too big (no convergene after ITMAX iteration)");
        return h;
}
...
double SNPHWE(int obs_hets, int obs_hom1, int obs_hom2)
{
	...
	if (obs_hom1 < 0 || obs_hom2 < 0 || obs_hets < 0) {
		...
		fprintf(stderr, "kc::SNPHWE: Current genotype configuration includes a negative count");
	}
	...
	if (het_probs == NULL) {
		fprintf(stderr, "kcerror::SNPHWE: Unable to allocate array for heterozygote probabilities" );
	}
