...
void estHMMFromFile_CHMM(CHMM hmm, int T, FILE *fp, int *niter, double *logprobinit, double *logprobfinal)
{
	...
	for (i = 1; i <= T; i++)
	{
		if (fscanf(fp, "%lf\t", &(O1[i])) != 1)
			fprintf(stderr, "ERROR: cannot read O1 from file");
		if (fscanf(fp, "%lf\t", &(O2[i])) != 1)
			fprintf(stderr, "ERROR: cannot read O2 from file");
		if (fscanf(fp, "%lf\n", &(pfb[i])) != 1)
			fprintf(stderr, "ERROR: cannot read PFB from file");
		if (fscanf(fp, "%i\n", &(snpdist[i])) != 1)
			fprintf(stderr, "ERROR: cannot read SNPDIST from file");
	}
...
/* calculate BAF */
{
	...
	{ // the hypothesis here is that stroma do not have LOH
		if (b == 0)
		{
			p += (1 - uf) * (1 - pfb) / 2;
		}
	...
	{ // the hypothesis here is that stroma has two copies
		if (b == 0)
		{
			p += (1 - uf) * (1 - pfb) * (1 - pfb) * (1 - pfb) / 2;
		}
...
/*  pprob is the LOG probability */
{
	...
	for (t = 1; t <= T - 1; t++)
	{
		...
		if (phmm->dist != 1)
			convertHMMTransition(phmm, A1, snpdist[t]);
		...
		for (j = 1; j <= phmm->N; j++)
		{
			...
			for (i = 1; i <= phmm->N; i++)
			{
				if (A1[i][j] < 0)
				{
					printf("FATAL ERROR at i=%i j=%i t=%i\n", i, j, t);
...
void BaumWelchNP_CHMM(CHMM *phmm, int T, double *O1, double *O2, double *pfb, int *snpdist, double **alpha, double **beta, double **gamma, int *pniter, double *plogprobinit, double *plogprobfinal)
{
	...
	for (t = 1; t <= T; t++)
	{
		...
		for (i = 1; i <= phmm->N; i++)
		{
			...
			{
				if (!phmm->NP_flag)
					std::cerr << "FATAL ERROR: detected CN probe but HMM model does not contain info for CN probe\n";
	...
	do
	{
		...
		for (i = 1; i <= phmm->N; i++)
		{
			for (j = 1; j <= phmm->N; j++)
			{
				...
				{
					if (i != 4)
						adjust = phmm->A[i][j] - inflation;
					if (i == 4)
						adjust = phmm->A[i][j] - inflation_s4;
					phmm->A[i][j] -= adjust;
					phmm->A[i][i] += adjust;
				}
			}
...
void ComputeXi_CHMM(CHMM *phmm, int T, double *O1, double *O2, double **biot, int *snpdist, double **alpha, double **beta, double ***xi)
{
	...
	for (t = 1; t <= T - 1; t++)
	{
		...
		for (i = 1; i <= phmm->N; i++)
			for (j = 1; j <= phmm->N; j++)
			{
				xi[t][i][j] = alpha[t][i] * beta[t + 1][j] * A1[i][j] * biot[j][t + 1];
				sum += xi[t][i][j];
			}
...
/// Return a default CHMM
CHMM GetDefaultCHMM()
{
	CHMM hmm;
	hmm.N = 5; // Number of states
	hmm.M = 5; //
...
void CopyCHMM(CHMM *phmm1, CHMM *phmm2)
{
	...
	if (phmm1->NP_flag)
	{
		...
		for (i = 1; i <= phmm1->N; i++)
			phmm2->B3_mean[i] = phmm1->B3_mean[i];
		for (i = 1; i <= phmm1->N; i++)
			phmm2->B3_sd[i] = phmm1->B3_sd[i];
...
double b4iot(int state, double *mean, double *sd, double uf, double pfb, int o1, int o2)
{
	...
	if (p > uf)
		p = uf;
	if (pfb < 0.01)
		pfb = 0.01; /*set up the minimum PFB value as 0.01*/

	if (state == 1)
	{
		if (o1)
			p += pdf_binomial(o1, o2, 0.25);
	}
	else if (state == 2)
	{
...
void callCNVFromFile_SEQ(CHMM hmm, int T, FILE *fp, int *mlstate, double gamma_k, double gamma_theta)
{
	...
	if (fgets(string, 1000, fp) == NULL)
		fprintf(stderr, "ERROR: cannot read first line from file");
	for (i = 1; i <= T; i++)
	{
		if (fscanf(fp, "%s\t", string) != 1)
			fprintf(stderr, "ERROR: cannot read string from file");
		if (fscanf(fp, "%lf\t", &(O1[i])) != 1)
			fprintf(stderr, "ERROR: cannot read O1 from file");
		if (fscanf(fp, "%i\t", &(O2[i])) != 1)
			fprintf(stderr, "ERROR: cannot read O2 from file");
		if (fscanf(fp, "%lf\t", &(pfb[i])) != 1)
			fprintf(stderr, "ERROR: cannot read PFB from file");
		if (fscanf(fp, "%i\n", &(length[i])) != 1)
			fprintf(stderr, "ERROR: cannot read LENGTH from file");
		if (i < 5)
...
void ViterbiLogNP_SEQ(CHMM *phmm, int T, double k, double theta, double *O1, int *O2, double *pfb, int *length, double **delta, int **psi, int *q)
{
	...
	phmm->B1_sd[1] = 1 / k;
	phmm->B1_mean[2] = k;
	phmm->B1_sd[2] = theta / 2.0;
	phmm->B1_mean[3] = k;
	phmm->B1_sd[3] = theta;
	phmm->B1_mean[4] = k;
	phmm->B1_sd[4] = theta;
	phmm->B1_mean[5] = k;
	phmm->B1_sd[5] = theta * 1.5;
	phmm->B1_mean[6] = k;
	phmm->B1_sd[6] = theta * 2.0;
	phmm->B3_mean[1] = k;
	phmm->B3_sd[1] = 1 / k;
	phmm->B3_mean[2] = k;
	phmm->B3_sd[2] = theta / 2.0;
	phmm->B3_mean[3] = k;
	phmm->B3_sd[3] = theta;
	phmm->B3_mean[4] = k;
	phmm->B3_sd[4] = theta;
	phmm->B3_mean[5] = k;
	phmm->B3_sd[5] = theta * 1.5;
	phmm->B3_mean[6] = k;
	phmm->B3_sd[6] = theta * 2.0;
	...
	for (t = 2; t <= T; t++)
	{
		...
		for (j = 1; j <= phmm->N; j++)
		{
			...
			psi[t][j] = maxvalind;
			if (t < 10)
				fprintf(stderr, "i=%i t=%i delta[t][i]=%f psi=%i\n", j, t, delta[t][j], psi[t][j]);
		}
	}
